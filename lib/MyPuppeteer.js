...
const WSE_PATH = path.resolve(__dirname, 'browser-ws-endpoint.txt');
...
class MyPuppeteer {
  constructor(options = {}) {
    ...
    // Register this browser with app
  ...
  async open(url, metadata = {}) {
    try {
      ...
      if (!this.browser) {
        let connected = false;
        if (fs.existsSync(WSE_PATH)) {
          try {
            const ws = fs.readFileSync(WSE_PATH, 'utf-8');
            this.browser = await puppeteer.connect({ browserWSEndpoint: ws });
            connected = true;
            console.log('Connected to existing browser at:', ws);
          } catch (err) {
            console.warn('Failed to connect to existing browser. Launching new one.');
          }
        }
        if (!connected) {
          this.browser = await puppeteer.launch(this.options);
          const ws = this.browser.wsEndpoint();
          fs.writeFileSync(WSE_PATH, ws);
          console.log('Browser launched at:', ws);
        }
      }
      ...
      // Check if this is an expected error during closing
      if (
        ...
        console.log(`Navigation interrupted (expected during browser closing): ${url}`);

        // Don't rethrow expected errors
        if (this.isOpen) {
          // Still consider this a successful navigation if we were previously open
          console.log(`Considering navigation to ${url} successful despite interruption`);
        }
      } else {
        // Only log and rethrow unexpected errors
  ...
  async close(force = false) {
    // Return early if browser is already null
    if (!this.browser) {
      console.log('Browser already closed, skipping close operation');
      return;
    }

    try {
      // Set a local reference and immediately null the instance property
      // to prevent multiple close attempts
      const browser = this.browser;
      this.browser = force ? null : this.browser;
      this.isOpen = false;
      ...
      // Regular page close (not browser) if not forced
      if (!force && this.page && !this.page.isClosed()) {
        try {
          await this.page.close();
        } catch (pageCloseErr) {
          console.warn('Error closing page:', pageCloseErr.message);
        }
      }

      // Regular browser close if we still have a reference and force requested
      if (force && browser) {
